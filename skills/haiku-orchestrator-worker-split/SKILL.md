...
- If the worker is also a Haiku agent and must return structured JSON, apply the `haiku-structured-return-protocol` skill to both the worker's instructions and the orchestrator's loop. Haiku workers are prone to returning free text instead of the expected JSON envelope, causing the orchestrator loop to silently misroute via the else/default branch.
