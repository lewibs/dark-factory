...
- `jq -s '.[0] * .[1]'` does a shallow merge. Nested objects are replaced wholesale, not deeply merged. Array fields are silently overwritten — successive sub-agent patches would drop all prior entries. If a brain-patch field is an array that multiple agents append to (e.g. `notes`, `artifacts`), use the concatenation pattern in the `brain-patch-array-field-merge` skill.
