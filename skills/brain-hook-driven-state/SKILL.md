...
- Hook-driven steps (phases advanced by the hook itself rather than by an Agent tool call) must still be declared in brain.json's `phases` object at the correct sequence position. For example, a `docs-running`/`docs-complete` pair triggered by the post-hook after `worker-complete` must appear between `worker-complete` and `review-running` in the declaration — otherwise the pre-hook's sequencer will try to advance them as if they were Agent-driven steps. See the `invoke-claude-from-hook` skill for the full pattern.
