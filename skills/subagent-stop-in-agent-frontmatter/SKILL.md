...
- **Do NOT reuse `commit-on-subagent-stop.sh` for new commands.** That script contains a hardcoded `case` statement for exactly three manufacture pipeline agents (`skeleton-agent`, `testing-agent`, `implementation-agent`). If a new command's orchestrator agent name is not in that list, the script silently exits 0 via the `*` catch-all — the commit never runs and no error is reported. When adding a new command that needs a SubagentStop commit hook, create a dedicated script (e.g., `agents/dark-factory/scripts/commit-<command>-docs.sh`) that reads the expected agent name and commits the appropriate files. Use `commit-investigation-docs.sh` as a reference implementation.
