...
- **Multi-turn loops compound the problem.** A caller that loops on a Haiku agent (spawn → receive → check status → re-spawn) loses the structured-return context across iterations. Haiku does not reliably maintain the return protocol across multiple invocations in a loop — it may return correctly on iteration 1 and return free text on iteration 2 because it treats each invocation as a new context. The fix is single-invocation design: instead of a status-object protocol with a multi-turn loop in the caller, give the Haiku agent `AskUserQuestion` directly (if it is at depth 2) so it handles its own interaction without returning through the caller. See `askuserquestion-depth-limit` for depth constraints on `AskUserQuestion`.
