...
- **A depth-2 agent can call `AskUserQuestion` directly — no message-passing protocol needed.** If the agent that needs to ask the user a question is itself at depth 2 (e.g., `feature-agent` invoked by the CLI), it should call `AskUserQuestion` directly rather than returning a `{ status: "question", ... }` object to its caller and relying on the caller to relay it. The message-passing protocol adds complexity and a Haiku reliability failure point. When `AskUserQuestion` is in the agent's `tools:` frontmatter and the agent is at depth 2, direct invocation is always preferable to a status-object relay through a parent loop. This was confirmed when `dark-factory-agent`'s multi-turn loop (which relayed `status:question` objects) was removed and `feature-agent` was given `AskUserQuestion` directly — the interaction became reliable.
