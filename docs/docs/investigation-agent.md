...
- Deploy command:
  ...
  ```
- Notes: The agent uses two skills: `agents/documentation/skills/investigate/SKILL.md` for codebase exploration and `agents/documentation/skills/documentation/SKILL.md` for writing documentation in the required template format. Tools include `Read`, `Grep`, `Glob`, `Bash`, `Write`, and `Edit`. Allowed bash commands are restricted to `find *`, `grep -r *`, and `ls *`. The agent declares a `SubagentStop` hook in its YAML frontmatter pointing to `agents/dark-factory/scripts/commit-investigation-docs.sh`, which stages and commits any new or updated files under `docs/docs/` when the agent finishes.
