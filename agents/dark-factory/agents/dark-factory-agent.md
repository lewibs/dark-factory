...
dark-factory-agent(taskDescription, taskName):
  ...
  # Batch mode: check DARK_FACTORY_BATCH_MODE (default: sync)
  ...
  if batchMode == "sync":
    ...
    # Try quick poll (10s), continue regardless
    docJobId = queue_batch_job("update-documentation-agent", {planFilePath})
    ...
    # Default fallback: synchronous invocation (pre-batch behavior)
    invoke update-documentation-agent({ planFilePath })

  # Step 9 — skill update (non-fatal)
  try:
    if batchMode == "sync":
      skillJobId = queue_batch_job("skill-update-agent", {planFilePath, workDir: WORK_DIR, taskSummary: taskDescription})
      ...
      # Default fallback: synchronous invocation
...
Modes:
- `sync` (default): Queue jobs, block until completion, proceed with PR. 50% cost savings, 100% backward compatible.
- `async`: Queue jobs, proceed with PR immediately. Faster PR opening; docs/skills may not be complete when PR opens.
- `poll`: Try quick poll (10s), continue regardless. Best-effort results.

If `DARK_FACTORY_BATCH_MODE` is unset or set to any other value, falls back to direct synchronous invocation (pre-batch behavior).
...
- Backward Compatibility: If Batch API is unavailable, fall back to synchronous invocation.
