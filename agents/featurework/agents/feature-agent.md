...
feature-agent(taskDescription, planPath):
  ...
  if phase == "flows":
    ...
    LOOP over each flow in allFlows:
      currentFlow = next unapproved flow from flow-state-manager
      if no unapproved flow: BREAK
      ...
      if answer == "Approve — continue to next flow":
        ...
        # Loop again to re-show the updated flow
